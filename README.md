...
**Phases 0 through 5 are complete; Phase 6 is in progress through PF-067:**
...
- **PF-060** replaced the API shell with a Fastify service framework, request-identified liveness
  and database readiness, development-only generated OpenAPI, bounded problem responses, and
  independent fixed-workspace web-owner, MCP-read-only, and webhook credential guards with
  cross-boundary rejection.
- **PF-061** added bounded fixed-workspace web-owner reads for accounts, credit cards, card bills,
...
- **PF-063** added bounded fixed-workspace web-owner category, canonical-merchant, and classification-
  rule management. Global categories remain immutable, merchant merges atomically rewire scoped
...
- **PF-066** added exact card-bill reconciliation summaries for web/MCP reads plus web-owner-only
  candidate generation and audited confirm/reject commands. Candidates require configured currency
  tolerance, a ±2-day window, a live deposit-account outflow, and bill-payment role evidence;
  PostgreSQL retains competing/rejected evidence and enforces one active economic-event match.
- **PF-067** added currency-separated confirmed installment commitment estimates over the canonical
  view, with exact remaining/monthly components, explicit monthly-from-purchase-date assumptions,
  unallocated/conversion/freshness warnings, identifier-free web/MCP analytics, and a web-only card
  review route that keeps candidate/review series outside committed totals.
...
management, spending/cash-flow analytics, period comparison, bill reconciliation, and installment-
commitment API.
